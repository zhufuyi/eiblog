...
func testConnectElasticsearch(url string) error {
	resp, err := http.Get(url + "/_cat/health")
	if err != nil {
		return err
	}
	...
	logd.Infof("testConnectElasticsearch resp body = [%s]\n", string(body))
	if resp.StatusCode != http.StatusOK {
		return fmt.Errorf("testConnectElasticsearch resp error, statusCode = %d", resp.StatusCode)
	}
	return nil
}
...
func InitElasticsearch(url string) error {
	err := testConnectElasticsearch(url)
	if err != nil {
		return err
	}

	es = &ElasticService{url: url, c: new(http.Client)}
	initIndex()
	return nil
}
